    ...
    _isRunning(true)
{
    ...
    // TODO
    //Serial.printf("*** NeoPixelAnimator _countAnimations: %d\n", _countAnimations); delay(200);
}
		...
		AnimUpdate* animUpdate)
{
    ...
    // TODO
    //Serial.printf("*** StartAnimation _clAnimUpdate: 0x%08x = d:%d - c:%d\n", uint32_t(animUpdate), duration, indexAnimation); delay(200);
        ...
        if (delta >= _timeScale)
        {
            ...
            for (uint16_t iAnim = 0; iAnim < _countAnimations; iAnim++)
            {
                ...
                if (pAnim->_remaining > delta)
                {
                    ...
                    //fnUpdate(param);
                    if (pAnim->_clAnimUpdate != nullptr)
                    {
                    	// TODO
                    	//Serial.printf("*** Started: Call_clAnimUpdate: 0x%08x\n", uint32_t(pAnim->_clAnimUpdate)); delay(200);
                    	pAnim->_clAnimUpdate->animUpdate(param);
                    }
                ...
                else if (pAnim->_remaining > 0)
                {
                    ...
                    {
                       	// TODO
                        	//Serial.printf("*** Completed: Call_clAnimUpdate: 0x%08x\n", uint32_t(pAnim->_clAnimUpdate)); delay(200);
                    	pAnim->_clAnimUpdate->animUpdate(param);
                    }
                    if (param.state == AnimationState_GoToRestart)
                    {
                    	RestartAnimation(param.index);
                    }
                }
            }
