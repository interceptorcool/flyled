...
public:
    ...
    void SetPixelColor(uint16_t indexPixel, typename T_COLOR_FEATURE::ColorObject color, float gradient = 1.0)
    {
    	if (_brightness)
        {
    		//Serial.print("SPC: g");Serial.println(gradient);
            ...
            {
                uint16_t value = *ptr;
                *ptr++ = (value * uint8_t(float(float(_brightness) * gradient))) >> 8;
            }
        }
