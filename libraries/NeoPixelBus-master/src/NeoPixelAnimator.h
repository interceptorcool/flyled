...
#undef min
#include <functional>
typedef std::function<void(AnimationParam& param)> AnimUpdateCallback;
...
{
public:
	virtual void animUpdate(AnimationParam& param) = 0;
	virtual ~AnimUpdate() {};
};
...
public:
    ...
    void StartAnimation(uint16_t indexAnimation, uint16_t duration, AnimUpdate* animUpdate);
    ...
    void RestartAnimation(uint16_t indexAnimation)
    {
        ...
        // TODO
        //Serial.printf("*** RestartAnimation _clAnimUpdate: 0x%08x\n", uint32_t(_animations[indexAnimation]._clAnimUpdate)); delay(200);

        StartAnimation(indexAnimation, _animations[indexAnimation]._duration, (_animations[indexAnimation]._clAnimUpdate));
    }
...
private:
    struct AnimationContext
    {
        ...
        ~AnimationContext()
        {
        	if (_clAnimUpdate != NULL)
        		// TODO
        		//Serial.printf("*** delete _clAnimUpdate: 0x%08x\n", uint32_t(_clAnimUpdate)); delay(200);

        		delete _clAnimUpdate;
        }

        //void StartAnimation(uint16_t duration, AnimUpdateCallback animUpdate)
        void StartAnimation(uint16_t duration, AnimUpdate* animUpdate)
        {
            _duration = duration;
            _remaining = duration;
            _clAnimUpdate = animUpdate;
